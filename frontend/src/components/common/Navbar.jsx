  ...
  return (
    <header className={`${landing ? 'absolute' : 'sticky backdrop-blur-md'} top-0 z-[60] h-16 w-full bg-transparent px-3 text-slate-950 sm:px-5`}>
      <div className="relative mx-auto flex h-full max-w-[1440px] items-center justify-between gap-4">
        ...
        <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-8 lg:flex xl:gap-11" aria-label={landing ? 'Landing navigation' : 'Application navigation'}>
        ...
        <button type="button" onClick={() => setMobileOpen((open) => !open)} className="focus-ring grid size-10 place-items-center rounded-full bg-white/55 text-emerald-950 backdrop-blur-sm lg:hidden" aria-expanded={mobileOpen} aria-label="Toggle navigation">{mobileOpen ? <X size={18} /> : <Menu size={18} />}</button>
      </div>
