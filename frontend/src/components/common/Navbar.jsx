  ...
  return (
    <header className={`${landing ? 'fixed backdrop-blur-md' : workspaceMode ? 'pointer-events-none fixed' : 'sticky backdrop-blur-md'} top-0 z-[60] h-16 w-full bg-transparent px-3 text-slate-950 sm:px-5`}>
      <div className="relative mx-auto flex h-full max-w-[1440px] items-center justify-between gap-4">
        ...
        {workspaceMode && <Link to="/" className="focus-ring pointer-events-auto rounded-xl lg:hidden" aria-label="Nexora home"><LogoMark className="size-7" alt="" /></Link>}
      ...
      {mobileOpen && (
        <div className="pointer-events-auto absolute inset-x-3 top-16 overflow-hidden rounded-2xl border border-white/80 bg-[#f6f8f2]/95 p-3 shadow-[0_20px_60px_rgba(23,55,47,.18)] backdrop-blur-xl sm:inset-x-5 lg:hidden">
          <div className="grid gap-2">
            {landing && <><a href="#product" onClick={() => setMobileOpen(false)} className="focus-ring min-h-11 rounded-xl border border-emerald-950/10 bg-white/60 px-4 py-3 text-xs font-semibold">Product</a><a href="#how-it-works" onClick={() => setMobileOpen(false)} className="focus-ring min-h-11 rounded-xl border border-emerald-950/10 bg-white/60 px-4 py-3 text-xs font-semibold">How it works</a><a href="#safety" onClick={() => setMobileOpen(false)} className="focus-ring min-h-11 rounded-xl border border-emerald-950/10 bg-white/60 px-4 py-3 text-xs font-semibold">Safety</a></>}
            {!landing && <><button type="button" onClick={() => go('/buyer')} className="focus-ring flex min-h-11 items-center gap-2 rounded-xl border border-emerald-950/10 bg-white/60 px-4 py-3 text-left text-xs font-semibold"><Bot size={14} /> Shopping assistant</button><button type="button" onClick={() => go('/merchant')} className="focus-ring flex min-h-11 items-center gap-2 rounded-xl border border-emerald-950/10 bg-white/60 px-4 py-3 text-left text-xs font-semibold"><Store size={14} /> Seller workspace</button></>}
            ...
            {!workspaceMode && !loading && !user && <button type="button" onClick={() => go('/login?mode=signup')} className="focus-ring flex min-h-11 items-center justify-center gap-2 rounded-xl border border-violet-300 bg-violet-50 px-4 py-3 text-xs font-semibold text-violet-700"><UserPlus size={14} /> Sign up</button>}
            {!workspaceMode && !loading && <button type="button" onClick={() => { setMobileOpen(false); user ? signOut() : navigate('/login') }} className="focus-ring flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-xs font-semibold text-white">{user ? <LogOut size={14} /> : <LogIn size={14} />}{user ? 'Sign out' : 'Sign in'}</button>}
          </div>
        </div>
      )}
    </header>
