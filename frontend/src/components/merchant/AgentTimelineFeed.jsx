...
export default function AgentTimelineFeed({ events, expanded = false }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-xl shadow-slate-950/20">
      <header className="flex items-start justify-between"><div><div className="flex items-center gap-2"><Radio size={16} className="text-indigo-400" /><h2 className="text-sm font-semibold text-white">Agent activity stream</h2></div><p className="mt-1 text-[10px] text-slate-500">Recommendation reasons and backend transaction outcomes.</p></div><span className="flex items-center gap-1.5 border border-slate-700 bg-slate-950 px-2 py-1 font-mono text-[8px] text-slate-400">BACKEND EVENTS</span></header>
          ...
          return (
            <article key={event.id} className="relative flex gap-3.5">
              ...
              <div className="min-w-0 flex-1 border-b border-slate-800/70 pb-4 last:border-0">
                ...
                <p className="mt-1.5 text-[11px] leading-relaxed text-slate-300"><span className="font-semibold text-white">{event.agent}</span> recorded <span className="font-semibold text-white">{event.actionLabel ?? style.label}</span> for <span className="font-semibold text-white">{event.product}</span> · {event.buyer}</p>
