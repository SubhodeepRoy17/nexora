...
function SpecViewer({ product, onClose }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 top-20 z-50 grid place-items-end bg-slate-950/85 backdrop-blur-md sm:place-items-center sm:p-4" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="spec-title" className="modal-scroll max-h-full w-full overflow-y-auto rounded-t-3xl border border-slate-700 bg-slate-900 shadow-2xl sm:max-w-xl sm:rounded-3xl">
        <header className="flex items-start justify-between border-b border-slate-800 p-5">
          <div>
            <p className="mono-label text-indigo-400">Product details</p>
            <h2 id="spec-title" className="mt-2 text-base font-semibold text-white">
              {product.name}
            </h2>
          ...
          <button type="button" onClick={onClose} className="rounded-full border border-slate-700 p-2 text-slate-400 hover:text-white">
            <X size={15} />
          </button>
        </header>
        <div className="p-5">
          <div className="mb-4 grid gap-2 sm:grid-cols-2">
            {Object.entries(product.specs).map(([key, value]) => (
              <div key={key} className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
                <p className="font-mono text-[8px] uppercase tracking-wider text-slate-600">{key.replaceAll('_', ' ')}</p>
                <p className="mt-1.5 font-mono text-[10px] text-slate-200">{String(value)}</p>
              </div>
            ))}
          </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-xl shadow-slate-950/20">
        <header className="flex flex-col gap-4 border-b border-slate-800 p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-white">Product inventory</h2>
            </div>
            <p className="mt-1 text-[10px] text-slate-500">Manage the products shoppers can discover.</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <button type="button" onClick={onAdd} className="focus-ring flex items-center justify-center gap-2 rounded-xl bg-indigo-500 px-3.5 py-2.5 text-[10px] font-semibold text-white shadow-glow hover:bg-indigo-400">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] border-collapse text-left">
            <thead>
              <tr className="inventory-table-head border-b border-slate-800 bg-slate-900 font-mono text-[8px] uppercase tracking-wider text-slate-100">
            ...
            <tbody>
              {filtered.map((product) => (
                <tr key={product.id} className="border-b border-slate-200 bg-white text-[11px] transition even:bg-slate-100 last:border-0 hover:bg-violet-50">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="grid size-10 place-items-center rounded-xl border border-slate-700 bg-slate-950 text-slate-500">
                        <Package size={16} />
                      </div>
                      <div>
                        ...
                        <div className="mt-1.5 flex gap-1">
                          {product.tags.slice(0, 2).map((tag) => (
                            <span key={tag} className="rounded bg-indigo-500/10 px-1.5 py-0.5 font-mono text-[7px] text-indigo-300">
                              #{tag}
                            </span>
                          ))}
                        </div>
                  ...
                  <td className="px-4 py-4">
                    {editingPriceId === product.id ? (
                      <div className="flex items-center gap-1">
                        ...
                        <input autoFocus type="number" min="1" value={draftPrice} onChange={(event) => setDraftPrice(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && savePrice(product)} className="w-20 rounded-lg border border-indigo-500 bg-slate-950 px-2 py-1.5 font-mono text-[10px] text-white" />
                        <button type="button" onClick={() => savePrice(product)} className="rounded-lg bg-emerald-500/10 p-1.5 text-emerald-400">
                          <Check size={12} />
                        </button>
                      </div>
                    ) : (
                      <button type="button" onClick={() => beginPriceEdit(product)} className="group flex items-center gap-1.5 font-semibold text-slate-200">
                        {money(product.price)}
                  ...
                  <td className="px-4 py-4">
                    <span className={`inline-block rounded-full border px-2 py-1 font-mono text-[8px] ${product.stock === 0 ? 'border-[#DC143C]/25 bg-[#DC143C]/10 text-[#ff607f]' : product.stock < 8 ? 'border-amber-500/20 bg-amber-500/10 text-amber-400' : 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400'}`}>{product.stock === 0 ? 'OUT OF STOCK' : `${product.stock} IN STOCK`}</span>
                    <p className="mt-1.5 text-[8px] text-slate-600">Use Edit to enter an exact quantity.</p>
                  </td>
                  <td className="px-4 py-4">
                    ...
                    <p className={`mt-1 font-mono text-[7px] ${product.active ? 'text-emerald-400' : 'text-slate-600'}`}>{product.active ? 'VISIBLE' : 'HIDDEN'}</p>
                  </td>
                  <td className="px-4 py-4">
                    <p className="font-mono text-[9px] text-slate-300">{product.agentViews.toLocaleString()} views</p>
                    <p className="mt-1 font-mono text-[8px] text-emerald-400">{product.conversions} purchases</p>
                  </td>
                  <td className="px-4 py-4">
                    <button type="button" onClick={() => setSpecProduct(product)} className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-950 px-2.5 py-2 font-mono text-[8px] text-indigo-300 transition hover:border-indigo-500/50">
                      <Package size={12} /> View details
                    </button>
                  </td>
                  <td className="px-4 py-4">
                    <button type="button" onClick={() => onEdit(product)} aria-label={`Edit ${product.name}`} className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-700 hover:text-white">
                      <Edit3 size={14} />
                    </button>
                  </td>
                </tr>
        ...
        <footer className="flex items-center justify-between border-t border-slate-800 px-5 py-3">
          ...
          <p className="text-[9px] text-slate-500">
            <span className="text-emerald-400">{products.filter((product) => product.active).length}</span> visible to shoppers
          </p>
        </footer>
      </section>
