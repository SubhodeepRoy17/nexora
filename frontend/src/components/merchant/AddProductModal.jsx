...
const emptyForm = {
  name: '', sku: '', category: 'Mechanical Keyboard', price: '', compareAt: '', stock: '', layout: '75%', switches: '', keycaps: 'Double-shot PBT', wireless: 'Bluetooth 5.1', battery: '', hotSwappable: true,
}
...
export default function AddProductModal({ open, product, onClose, onSave }) {
  ...
  const submit = (event) => {
    ...
    const next = {
      ...product,
      id: product?.id ?? `prd-${form.sku.toLowerCase().replaceAll(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || Date.now()}`,
      name: form.name,
      sku: form.sku,
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-end bg-slate-950/85 backdrop-blur-md sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="product-modal-title" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <form onSubmit={submit} className="max-h-[92dvh] w-full overflow-y-auto rounded-t-3xl border border-slate-700 bg-slate-900 shadow-2xl sm:max-w-3xl sm:rounded-3xl">
        ...
        <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[1fr_.8fr]">
          <div>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-[10px] font-medium text-slate-400">SKU<input required value={form.sku} onChange={(event) => update('sku', event.target.value.toUpperCase())} placeholder="KB-K2P-84" className={inputClass} /></label>
              <label className="text-[10px] font-medium text-slate-400">Category<select value={form.category} onChange={(event) => update('category', event.target.value)} className={inputClass}><option>Mechanical Keyboard</option><option>Hall Effect Keyboard</option><option>Low Profile Keyboard</option><option>Wireless Mouse</option></select></label>
              ...
              <label className="text-[10px] font-medium text-slate-400">Compare-at price (₹)<input min="1" type="number" value={form.compareAt} onChange={(event) => update('compareAt', event.target.value)} className={inputClass} /></label>
              ...
              <label className="text-[10px] font-medium text-slate-400 sm:col-span-2">Switches<input required value={form.switches} onChange={(event) => update('switches', event.target.value)} placeholder="Gateron Brown Tactile" className={inputClass} /></label>
          ...
          <aside className="rounded-2xl border border-indigo-500/20 bg-slate-950/70 p-4">
            ...
            <p className="mt-3 text-[9px] leading-relaxed text-slate-600">This preview mirrors the structured payload an AI buyer agent can safely query. Backend schema validation will be added during API integration.</p>
          </aside>
        </div>
