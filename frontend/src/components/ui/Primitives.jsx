...
export function Eyebrow({ children, tone = 'violet', className = '' }) {
  const tones = {
    ...
    dark: 'border-slate-300 bg-white text-slate-700',
  }
...
export function SignalButton({ to, href, children, variant = 'primary', className = '', icon = true, ...props }) {
  const styles = {
    ...
    dark: 'border-slate-950 bg-slate-950 text-white hover:bg-violet-700',
  }
...
export function SectionHeading({ eyebrow, title, description, align = 'left', inverse = false }) {
  ...
  return (
    <div className={`flex max-w-3xl flex-col ${alignment}`}>
      ...
      <h2 className="mt-5 text-balance text-3xl font-semibold leading-[1.05] tracking-[-0.045em] text-slate-950 sm:text-5xl">{title}</h2>
      {description && <p className="mt-5 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">{description}</p>}
    </div>
  )
}
