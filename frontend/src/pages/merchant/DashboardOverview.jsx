...
export default function DashboardOverview({ analytics, inventory, events, onNavigate, merchantName }) {
  ...
  return (
    <div>
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Merchant performance metrics">{metrics.map(({ id, label, value, detail, icon: Icon }) => <article key={id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-xl shadow-slate-950/20"><span className={`grid size-9 place-items-center rounded-xl ${id === 'revenue' || id === 'growth' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-indigo-500/10 text-indigo-400'}`}><Icon size={17} /></span><p className="mt-5 text-[10px] text-slate-500">{label}</p><p className="mt-1 text-2xl font-semibold tracking-tight text-white">{value}</p><p className="mt-2 font-mono text-[8px] text-slate-600">{detail}</p></article>)}</section>

      <div className="mt-5 grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        <AgentTimelineFeed events={events} />
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5"><div className="flex items-center gap-2"><Sparkles size={15} className="text-indigo-400" /><h2 className="text-sm font-semibold text-white">Growth attribution</h2></div><dl className="mt-4 space-y-3 text-[10px]"><div className="flex justify-between"><dt className="text-slate-500">Offer responses</dt><dd className="text-white">{number(growth.responded_offers)} / {number(growth.offer_impressions)}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Accept rate</dt><dd className="text-white">{Number(growth.accept_rate_percent ?? 0).toFixed(2)}%</dd></div><div className="flex justify-between"><dt className="text-slate-500">Paid attachment rate</dt><dd className="text-white">{Number(growth.paid_attachment_rate_percent ?? 0).toFixed(2)}%</dd></div></dl><p className="mt-4 rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-3 text-[9px] leading-relaxed text-slate-400">{analytics?.growth?.attribution_note ?? 'No attribution data yet.'}</p><button type="button" onClick={() => onNavigate('insights')} className="mt-4 text-[10px] font-semibold text-indigo-400">View denominators and gaps →</button></section>
      </div>
    </div>
  )
}
