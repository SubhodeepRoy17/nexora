  ...
  return (
    <div>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex items-center gap-3 border border-slate-800 bg-slate-900 px-3.5 py-2.5"><span className="grid size-8 place-items-center bg-emerald-500/10 text-emerald-400"><Package size={14} /></span><div><p className="font-mono text-[8px] uppercase text-slate-600">Owner-scoped catalog</p><p className="mt-0.5 text-[10px] font-medium text-slate-300">{activeProducts} indexed · {stockUnits} available units</p></div></div>
      </div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        <div><div className="mb-2 flex justify-end"><DataFreshness updatedAt={timelineState.updatedAt} loading={timelineState.loading} staleAfterMs={20000} dark /></div><AgentTimelineFeed events={events} /></div>
        <section className="border border-slate-800 bg-slate-900 p-5"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-2"><Sparkles size={15} className="text-violet-400" /><h2 className="text-sm font-semibold text-white">Growth attribution</h2></div><DataFreshness updatedAt={analyticsState.updatedAt} loading={analyticsState.loading} staleAfterMs={30000} dark /></div><dl className="mt-4 space-y-3 text-[10px]"><div className="flex justify-between"><dt className="text-slate-500">Offer responses</dt><dd className="text-white">{number(growth.responded_offers)} / {number(growth.offer_impressions)}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Accept rate</dt><dd className="text-white">{Number(growth.accept_rate_percent ?? 0).toFixed(2)}%</dd></div><div className="flex justify-between"><dt className="text-slate-500">Paid attachment rate</dt><dd className="text-white">{Number(growth.paid_attachment_rate_percent ?? 0).toFixed(2)}%</dd></div></dl><p className="mt-4 border border-violet-500/20 bg-violet-500/5 p-3 text-[9px] leading-relaxed text-slate-400">{analytics?.growth?.attribution_note ?? 'No recorded attribution data yet.'}</p><button type="button" onClick={() => onNavigate('insights')} className="mt-4 text-[10px] font-semibold text-violet-400">View denominators and gaps →</button></section>
      </div>
      <div className="mt-5"><MerchantOperations orders={orders} workspace={workspace} state={operationsState} onRetry={onRetryOperations} /></div>
    </div>
  )
}
