...
describe('owner-scoped merchant workspace', () => {
  ...
  it('selects the authenticated merchant from backend workspace data without a client merchant scope', async () => {
    ...
    expect(screen.getByText('OWNER-SCOPED WORKSPACE')).toBeInTheDocument()
  })
  ...
  it('renders honest empty states for catalog, paid orders, timeline, and relationships', async () => {
    ...
    expect(screen.getByText('No compatibility relationships yet. The agent will correctly make no add-on offer.')).toBeInTheDocument()
    inventoryView.unmount()
    renderMerchant('/merchant')
    expect(await screen.findByText('No webhook-confirmed paid orders yet.')).toBeInTheDocument()
    expect(screen.getByText('No order activity yet. New checkout and webhook events will appear automatically.')).toBeInTheDocument()
  })
})
