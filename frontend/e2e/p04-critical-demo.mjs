...
try {
  ...
  await page.getByRole('heading', { name: 'Checkout paused' }).waitFor()
  await page.getByText(/You chose 6\. The limit is 5 per item/i).waitFor()
  await page.getByText(/No payment · No stock change/i).waitFor()
  ...
  const pay = page.getByRole('button', { name: /Approve & pay/i })
  await pay.focus()
  ...
  await Promise.all([logoutResponse, page.getByRole('menuitem', { name: 'Sign out' }).click()])
