...
ACP_BUYER_TOKEN_TTL_SECONDS = max(300, min(3600, int(os.getenv("ACP_BUYER_TOKEN_TTL_SECONDS", "900"))))
...
GROWTH_EXPERIMENT_ENABLED = os.getenv("GROWTH_EXPERIMENT_ENABLED", "True").lower() in {
    "1", "true", "yes", "on"
}
GROWTH_EXPERIMENT_KEY = os.getenv("GROWTH_EXPERIMENT_KEY", "addon-offer-v1")[:80]
GROWTH_EXPERIMENT_TREATMENT_BPS = min(
    10_000, max(0, int(os.getenv("GROWTH_EXPERIMENT_TREATMENT_BPS", "5000")))
)
GROWTH_EXPERIMENT_MIN_SAMPLE_PER_VARIANT = max(
    10, int(os.getenv("GROWTH_EXPERIMENT_MIN_SAMPLE_PER_VARIANT", "100"))
)
