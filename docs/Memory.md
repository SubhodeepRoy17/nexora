...
- Neon cutover preparation preserves `sslmode`, `channel_binding`, and supported routing options from `DATABASE_URL`; `docs/NeonRunbook.md` documents credential-safe backup, direct restore/migration, validation, deployment, and rollback. The active database has not been switched pending the user's Neon project and secret.
